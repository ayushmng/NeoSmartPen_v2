...
//		PendingIntent pendingIntent = PendingIntent.getBroadcast( this, 0, new Intent( "firmware_update" ), PendingIntent.FLAG_UPDATE_CURRENT );
    ...
    private void chkPermissions() {
        if (Build.VERSION.SDK_INT >= 23) {
            int gpsPermissionCheck = ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION);
            int coarsePermissionCheck = ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_COARSE_LOCATION);
            int bluetoothScanPermissionCheck = ActivityCompat.checkSelfPermission(this, Manifest.permission.BLUETOOTH_SCAN);
            int bluetoothConnectPermissionCheck = ActivityCompat.checkSelfPermission(this, Manifest.permission.BLUETOOTH_CONNECT);
            int bluetoothAdvPermissionCheck = ActivityCompat.checkSelfPermission(this, Manifest.permission.BLUETOOTH_ADVERTISE);
            ...
            if (gpsPermissionCheck == PackageManager.PERMISSION_DENIED || writeExternalPermissionCheck == PackageManager.PERMISSION_DENIED) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    if (bluetoothConnectPermissionCheck == PackageManager.PERMISSION_DENIED) {
                        permissions.add(Manifest.permission.BLUETOOTH_CONNECT);
                    }
                    if (bluetoothScanPermissionCheck == PackageManager.PERMISSION_DENIED) {
                        permissions.add(Manifest.permission.BLUETOOTH_SCAN);
                    }
                    if (bluetoothAdvPermissionCheck == PackageManager.PERMISSION_DENIED) {
                        permissions.add(Manifest.permission.BLUETOOTH_ADVERTISE);
                    }
                }
                requestPermissions(permissions.toArray(new String[permissions.size()]), REQ_GPS_EXTERNAL_PERMISSION);
                requestPermissions(permissions.toArray(new String[]{Manifest.permission.BLUETOOTH_CONNECT}), 2);
                requestPermissions(permissions.toArray(new String[]{Manifest.permission.BLUETOOTH_SCAN}), 2);
                requestPermissions(permissions.toArray(new String[]{Manifest.permission.BLUETOOTH_ADVERTISE}), 2);
            }
        }
    }
